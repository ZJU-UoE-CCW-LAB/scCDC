...
This `mislet_before` dataset is a matrix with `r dim(mislet_before)[1]` genes and `r dim(mislet_before)[2]` cells.

## Applying Seurat pipeline
...
## Detecting global contamination-causing genes (GCGs)
...
Using default settings, entropy divergence (the degree of contamination within each cluster) of qualified clusters for only GCGs will be returned with gene names. By determining the directories of plots and the table, the Expression-Entropy plots and the full contamination degree table (including uncontaminated genes) will be output into the directories.

Here, in this tutorial, the generated example plots and table will be output into your current working directory for simplicity.
...
```
Here, red dots represent candidate GCGs in each cluster while blue represents non-candidate GCGs.

## Comparing the entropy divergences

Using the full table of entropy divergences, we can compare the difference between contaminated genes (GCGs) and uncontaminated genes (non-GCGs). 
...
### Resetting the default assay and performing the normalization
...
If any problems occur when using *scCDC*, please report it on [issues](https://github.com/ZJU-UoE-CCW-LAB/scCDC/issues). For more specific issues, [email us](yihui.19@intl.zju.edu.cn)!
