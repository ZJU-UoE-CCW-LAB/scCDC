...
#' @import pROC
#'
#' @examples AUC_res <- test_roc(exp_value, cell_type)
simple_roc <- function(exp,class){
  # match and sort
  names(exp) <- class
  exp_class <- sort(exp)
  # draw the roc
  curve <- roc(names(exp_class), exp_class)
  AUC <- curve[['auc']]
  return(AUC)
}
...
Cal_AUCs <- function(object,gene){
  ...
  for (cluster in cls) {
    ...
    class <- c(rep(0, length(cls_cells)), rep(1, length(neg_cells)))
...
#' @param object a clustered SeuratObject
#' @param gene a gene within the input SeuratObject
#' @param eGCG_aucs a named vector of the AUROC values between the each cluster and the
#' cluster with the lowest expression level
#' @param auc_thres the AUROC threshold to determine the boundary between eGCG_positive
#' and eGCG_negative clusters
#'
#' @return the count value of the threshold (best cutting-point)
#'
#' @import Seurat pROC ddpcr
#'
#' @examples count_thres <- Cal_thres(seuratobj, gene, eGCG_aucs, auc_thres)
Cal_thres <- function(object,gene,eGCG_aucs,auc_thres){
  ...
  quiet(curve<-roc(names(exps),exps))
...
#' @param auc_thres the AUROC threshold to determine the boundary between eGCG_positive
#' and eGCG_negative clusters (Default as 0.9, 90%)
#'
...
#' @export
#'
#' @examples seuratobj_corrected <- ContaminationCorrection(seuratobj, cont_genes, 
#' auc_thres = 0.9)
