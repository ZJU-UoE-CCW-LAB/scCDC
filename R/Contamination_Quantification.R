...
ContaminationQuantification <- function(object,cont_genes,
  ...
  if (max(index_vals)>-7){
    message(paste0('The maximum log contamination ratio is:',round(max(index_vals),2),', which indicates high contamination in this dataset. scCDC is highly recommended to be applied.'))
  }else{
    message(paste0('The maximum log contamination ratio is:',round(max(index_vals),2),', which indicates low contamination in this dataset. Either DecontX or scCDC is recommended to be applied.'))
  }
